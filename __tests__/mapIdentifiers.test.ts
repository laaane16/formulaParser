...
describe('Parser.mapIdentifiers', () => {
  ...
  test('should map identifiers from dbId to id ({3} should remain unchanged)', () => {
  ...
  test('should map identifiers from dbId to columnName ({3} should become {field3})', () => {
    const parser = new Parser(expression, mockFields);
  ...
  test('should throw an error if variable not found using "from" field', () => {
    const parser = new Parser('{999} + 10', mockFields);

    expect(() => parser.mapIdentifiers({ from: 'dbId', to: 'id' })).toThrow();
  });

  test('should throw an error if target field ("to") does not exist', () => {
  ...
  test('map ids should saving whitespaces', () => {
